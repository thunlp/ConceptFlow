...
from dataset import ConceptFlowDataset
...
def get_data(config, word2id, entity2id):
    collate = _build_collate(config, word2id, entity2id)

    train_loader = None
    if config.is_train:
        train_dataset = ConceptFlowDataset('trainset.txt',config)
        train_loader = DataLoader(train_dataset,batch_size=config.batch_size,collate_fn=collate)
        
    test_dataset = ConceptFlowDataset('testset.txt',config)
    test_loader = DataLoader(test_dataset,batch_size=config.batch_size,collate_fn=collate)

    return train_loader,test_loader
            
def _build_collate(config, word2id, entity2id):
